...
class Enemy extends FlxSprite implements IHitable {
    ...
    public function new(x:Float, y:Float) {
        ...
        loadGraphic('assets/monster1_tilesheet.png', true, 192, 192);
        immovable = false;

        _currLevel = -1;
        offset.add(Facade.I.monsterOffsets[0].x, Facade.I.monsterOffsets[0].y);
        ...
        animation.add(ANIM_MOVE, [0, 1, 2, 3], 4);
//        addAnimation(ANIM_ATTACK, [0, 0, 4, 5, 6, 7, 0, 0, 0, 0, 0], Math.floor(_attackSpeed));
        animation.add(ANIM_DEATH, [8, 9, 10, 11, 12, 12], 7);
        animation.add(ANIM_STAGGER, [8, 9, 8], 12);
