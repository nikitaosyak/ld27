...
class TmxMap
{
	...
	public var tilesets     : StringMap<TmxTileSet>;
	public var layers       : StringMap<TmxLayer>;
	public var objectGroups : StringMap<TmxObjectGroup>;
...
//		else if (Std.is(data, ByteArray)) source = new Fast(Xml.parse(data.toString()));
		...
		tilesets = new StringMap<TmxTileSet>();
		layers = new StringMap<TmxLayer>();
		objectGroups = new StringMap<TmxObjectGroup>();
